...
async function leaveRecruitAPI(_id, myAccountName, recruitData) {
    try {
        const token = await masterTokenAPI();
        const leaveArray = JSON.parse(recruitData.link)[2].filter((data) => {
          if(data !== myAccountName) return data;
        })
...
export default leaveRecruitAPI;
